...
func (r *userRepo) CreateUser(ctx context.Context, name, email, password string) (*biz.User, error) {
	...
	if r.data.redis.HMSet(ctx, u.Email, u).Err() != nil {
		return nil, errors.New(int(pb.ErrorReason_VALIDATIONS), "user", fmt.Sprintf("redis err(%+v)", email))
	}
	if r.data.db.Create(u).Error != nil {
		return nil, errors.New(int(pb.ErrorReason_VALIDATIONS), "user", fmt.Sprintf("db err(%+v)", email))
	}
	return u, nil
}
