...
func (r *userRepo) CreateUser(ctx context.Context, name, email, password string) (*biz.User, error) {
    ...
    if r.data.redis.HMSet(ctx, fmt.Sprintf("HAccount:%d", u.ID), u).Err() != nil {
        return nil, errors.New(500, "user", fmt.Sprintf("redis err(%+v)", email))
    }
    if r.data.db.Create(u).Error != nil {
        return nil, errors.New(500, "user", fmt.Sprintf("db err(%+v)", email))
    }
    return u, nil
}
